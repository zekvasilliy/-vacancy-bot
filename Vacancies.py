    ...
    for a in listing_links:
        ...
        for _ in range(6):
            ...
            if not title or len(title) < 8:
                parsed_title = None
                m = re.search(r"([A-ZƏĞIİÖŞÇÜa-zəğıiöşçü0-9][^\n]{8,160})\s+(?:Razılaşma ilə|\d{1,2}\s+[A-Za-zƏĞIİÖŞÇÜa-zəğıiöşçü]+\s+\d{4})", context)
                if m:
                    parsed_title = clean_title(m.group(1))
                elif " Görəcəyiniz işlər:" in context:
                    parsed_title = clean_title(context.split(" Görəcəyiniz işlər:", 1)[0])
                elif " Gördüyünüz işlər:" in context:
                    parsed_title = clean_title(context.split(" Gördüyünüz işlər:", 1)[0])
